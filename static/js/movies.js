$.Brainstage.registerPlugin('Plugins/Movies', function(Plugin){
	...
	container.buildSplitView({
		buildListCellFn:
			function(tableRow, item){
				...
				tableRow.subordinate( 'td', ""+item.review_count );
			},
		buildDetailsFn:
			function(detailsContainer, item){
				...
				detailsContainer.find('.url').text( item.url ).attr('href', item.url);
			},
		apiCallFinishedFn:
			function(){},
		savedSuccessFn:
