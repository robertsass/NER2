...
(function(doc) {
  ...
  Pen.prototype.toolbar = function() {
    ...
    for(var i = 0, list = this.config.list; i < list.length; i++) {
		...
		icons += '<a class="' + klass + '" data-action="' + name + '">' + (name.match(/^h[1-6]|p$/i) ? name.toUpperCase() : '') + '</a>';
		if((name === 'createlink')) {
  ...
  Pen.prototype.actions = function() {
    ...
    wrap = function(tag) {
      ...
      /* SPECIAL EXTENSION >> */
      if( tag == 'latex' ) {
	      val = '<latex source=""><span class="source">'+ document.getSelection() +'</span><span class="graphic"></span></latex>';
      }
      /* << SPECIAL-EXTENSION */
